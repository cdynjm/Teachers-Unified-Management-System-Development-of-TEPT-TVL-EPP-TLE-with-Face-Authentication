        ...
        @foreach ($subjects as $index => $sub)
            @if($sub->strand == 1)
                @if($sub->Strands->semester == 1 && $sub->Strands->yearLevel == $yearLevel)
                    <tr>
                        <td>
                            <a wire:navigate href="{{ route('pro-meds', ['id' => $aes->encrypt($sub->id), 'type' => 2, 'type2' => 1]) }}" class="ms-3 text-sm fw-bolder mb-0">{{ $sub->Strands->subject }}</a>
                        </td>
            ...
            @else
                @if($sub->SeniorHighSchoolSubjects->semester == 1 && $sub->SeniorHighSchoolSubjects->yearLevel == $yearLevel)
                    <tr>
                        <td>
                            <a wire:navigate href="{{ route('pro-meds', ['id' => $aes->encrypt($sub->id), 'type' => 2, 'type2' => 0]) }}" class="ms-3 text-sm fw-bolder mb-0">{{ $sub->SeniorHighSchoolSubjects->subject }}</a>
                        </td>
