<table class="table align-items-center mb-0" id="elementary-school-data-result">
    <thead>
        <tr>
    ...
    <tbody>
       ...
       @foreach ($elementarySchools->sortBy('school') as $sc)
           <tr>
                <td class="text-center text-sm" id="{{ $aes->encrypt($sc->id) }}" school="{{ $aes->encrypt(1) }}">
                ...
                <td class="text-center">
                    <a wire:navigate href="{{ route('edit-school', ['school' => $aes->encrypt(1), 'id' => $aes->encrypt($sc->id)]) }}?key={{ \Str::random(50) }}" class="text-secondary font-weight-bold text-xs me-2"
                        data-toggle="tooltip">
